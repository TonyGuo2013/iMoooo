...
class YafDebug
{
    ...
    public static function error($v, $logFilePrefix = 'err')
    {
        if(YAF_LOG_LEVEL_ERROR >= self::$LOG_LEVEL)
        {
            $traces = debug_backtrace(0, 1);
            $invoker = $traces[0];
            self::write($v, YAF_LOG_LEVEL_ERROR, $invoker['file'], $invoker['line'], $logFilePrefix);
        }
    }

    public static function log($v, $logFilePrefix = 'debug')
    {
        if(YAF_LOG_LEVEL_DEBUG >= self::$LOG_LEVEL)
        {
            $traces = debug_backtrace(0);
            $invoker = $traces[0];
            self::write($v, YAF_LOG_LEVEL_DEBUG, $invoker['file'], $invoker['line'], $logFilePrefix);
        }
    }
    ...
    public static function cookieLog($v)
    {
        self::log($v, "cookie");
    }
...
//        }
    ...
    private static function write(&$info , $level, $file = '', $line='', $logFilePrefix='')
    {
        if (strlen ($file) > 0)
        ...
        if (!empty($logFilePrefix))
        {
            $logFilePrefix .= '_';
        }

        $path =  sprintf('%s/%s%d.log', $path, $logFilePrefix, $hour);
//        var_dump($path);
